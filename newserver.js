...
server.post('/slack-clone/api/login', (req, res) => {
...
server.get('/slack-clone/api/me', protected, (req, res) => {
	db('userInfo')
...
server.get('/slack-clone/api/users', protected, checkRole('PM'), (req, res) => {
	db('userInfo')
		.select('id', 'username', 'password') // ***************************** added password to the select
		.then((users) => {
			res.json(users);
...
server.post('/slack-clone/api/register', (req, res) => {
	// grab username and password from body
	const creds = req.body;
...
server.get('/slack-clone/api/logout', (req, res) => {
	const token = req.headers.authorization;
	if (token) {
	}
});
